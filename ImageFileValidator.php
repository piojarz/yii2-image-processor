...
/**
 * Description of ImageFileValidator
 *
 * @author mlapko
 * 
 */
class ImageFileValidator
{
    ...
    {
        $image = \Yii::$app->get(self::imageProcessor)->getImageHandler();
        try {
            $name = pathinfo($filename, PATHINFO_BASENAME);
            $image->load($filename);
            if (self::minWidth !== null && $image->getWidth() < self::minWidth) {
                $message = self::tooSmallWidth !== null ? self::tooSmallWidth : Yii::t('app', 'The image "{file}" is too small. Its width cannot be smaller than {limit} px.');
            ...
            if (self::minHeight !== null && $image->getHeight() < self::minHeight) {
                $message = self::tooSmallHeight !== null ? self::tooSmallHeight : Yii::t('app', 'The image "{file}" is too small. Its height cannot be smaller than {limit} px.');
            ...
            if (self::maxWidth !== null && $image->getWidth() > self::maxWidth) {
                $message = self::tooLargeWidth !== null ? self::tooLargeWidth : Yii::t('app', 'The image "{file}" is too large. Its width cannot exceed {limit} px.');
            ...
            if (self::maxHeight !== null && $image->getHeight() > self::maxHeight) {
                $message = self::tooLargeHeight !== null ? self::tooLargeHeight : Yii::t('app', 'The image "{file}" is too large. Its height cannot exceed {limit} px.');
                self::addError($object, $attribute, $message, array('{file}'  => $name, '{limit}' => self::maxHeight));
            }
        } catch (Exception $exc) {
            $message = self::invalidImage !==null ? self::invalidImage : Yii::t('app', 'Invalid image.');
